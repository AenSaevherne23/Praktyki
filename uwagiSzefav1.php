...
while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
    ...
    // Obliczenie domyslnej ceny sprzedaży
    ...
    // gdy cen konkurencji > 3
    if($cs_domyslna <= $cena_min)
    {
        // jeżeli ilośc cen min > 1 to zastosuj
        $ocs = $cena_min;
        // inaczej jeżeli średnia > dominanta
        $ocs = $dominanta;
        // inaczej średnia
    }
    else
    ...
    if ($cs_domyslna > $aktualna_cena_sprzedazy) {
